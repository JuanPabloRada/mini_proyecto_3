...
public class VentanaDuelo extends JFrame {
    ...
    //Crea un boton estilizado para la columna de acciones. Se devuelve el boton
    // sin listeners adicionales; el codigo llamante conecta los listeners.
    ...
    // Metodos publicos llamados por el Controller 

    // Añade una linea al registro de batalla. Se asegura de mantener el caret
    //  al final para que el usuario vea siempre el ultimo mensaje.
    ...
    // Reresca toda la interfaz tomando el estado actual del CampoBatalla del controller. Se actualizan etiquetas, colores y paneles de cartas
    public void actualizarUI() {
        CampoBatalla campo = controller.getCampo();
        Jugador activo   = campo.getJugadorActivo();
        Jugador oponente = campo.getOponente();

        // Mostrar numero de turno y nombre del jugador activo.
        lblTurno.setText("Turno " + campo.getTurnoActual() + "  —  Turno de: " + activo.getNombre().toUpperCase());

        // Abajo = activo, Arriba = oponente: actualizar todas las etiquetas visibles.
        ...
        // Colorear los LP segun umbrales para dar feedback visual (verde, amarillo, rojo).
        colorearLP(lblLpJ1, activo.getLp());
        colorearLP(lblLpJ2, oponente.getLp());

        // Refrescar visualmente los paneles que contienen las tarjetas de monstruos.
        refrescarPanelCampo(panelCampoJ1, activo.getCampo());
        refrescarPanelCampo(panelCampoJ2, oponente.getCampo());

        // Habilitar/deshabilitar botones en funcion del estado del jugador activo.
    ...
    // Cambia el color de la etiqueta LP segun el valor: alto=verde, medio=amarillo, bajo=rojo.
    ...
    // Refresca el panel que muestra las tarjetas de monstruos. Si no hay monstruos, añade una etiqueta indicando que el campo esta vacio.
    ...
    private void refrescarPanelCampo(JPanel panel, List<CartaMonstruo> monstruos) {
        ...
        // Revalidar y repintar para que Swing actualice la UI correctamente.
        panel.revalidate();
        panel.repaint();
    }

    // Crea un componente visual (tarjeta) que representa un `CartaMonstruo`. La tarjeta incluye nombre, nivel, ATK/DEF, modo (ATK/DEF) y si ha atacado.
    private JPanel crearTarjetaMonstruo(CartaMonstruo m) {
        ...
        // Color del borde depende si esta en modo defensa o ataque.
        ...
        // Indicador de modo (DEF/ATK) con color acorde.
        ...
        // Indicador si puede atacar (marca verde) o ya ataco (gris).
    ...
    // Muestra un dialogo modal anunciando el ganador. Desactiva botones para evitar mas acciones y ofrece iniciar una nueva partida.
    ...
    public void mostrarGanador() {
        ...
        // Actualizar UI antes de mostrar el dialogo final.
        actualizarUI();
        ...
        btnNuevo.addActionListener(e -> {
            // Al crear una nueva VentanaInicio se reinicia el flujo de la aplicación.
            dialogo.dispose();
